...
export const fetchVisitors = async (dashboardId = 1) => {
  try {
    ...
    console.error(`대시보드 ID ${dashboardId}의 방문객 목록을 가져오는데 실패했습니다:`, error);
    // 오류 발생 시 빈 배열 반환
    return [];
  }
};
...
export const fetchVisitorById = async (visitorId, dashboardId = 1) => {
  try {
    ...
    console.error(`대시보드 ID ${dashboardId}의 방문객 ID ${visitorId} 정보를 가져오는데 실패했습니다:`, error);
    // 오류 발생 시 null 반환
    return null;
  }
};
...
export const createVisitor = async (visitorData, dashboardId = 1) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/visitors/${dashboardId}`, visitorData);
    return response.data;
  } catch (error) {
    console.error(`대시보드 ID ${dashboardId}에 방문객 생성에 실패했습니다:`, error);
    throw error;
  }
};
...
export const deleteVisitor = async (visitorId, dashboardId = 1) => {
  try {
    const response = await axios.delete(`${API_BASE_URL}/visitors/${dashboardId}/${visitorId}`);
    return response.data;
  } catch (error) {
